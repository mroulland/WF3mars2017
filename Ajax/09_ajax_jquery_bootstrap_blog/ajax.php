...
if($mode == 'enregistrer'){
    if(!empty($_POST['titre']) && !empty($_POST['auteur']) && !empty($_POST['contenu'])) // permet de se prémunir contre les injections dans l'inspecteur
    {
    ...
    // mettre en place une fonction ajax qui envoie l'argument mode=liste et qui affiche tous les articles dans l'élément avecl'id liste
    // => setInterval
    ...
    while($article = $liste_article->fetch(PDO::FETCH_ASSOC)){
        ...
        $tab['resultat'] .= "<div class='panel panel-heading'><h2>" . $article['titre'] . "</h2></div>";
        $tab['resultat'] .= "<div class='pannel-body'>";
